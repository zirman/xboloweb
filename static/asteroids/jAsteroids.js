var jAsteroids = function () {
  if (!Modernizr.canvas) {
    return;
  }

  var canvas = document.getElementById("jAsteroidsCanvas");
  ...
  var gameStateNewLevel = function () {
    rocks = [];
    saucers = [];
    playerMissiles = [];
    particles = [];
  ...
  var gameStatePlayerStart = function () {
    fillBackground();
    renderScoreBoard();
  ...
  var gameStatePlayLevel = function () {
    checkKeys();
    update();
    render();
    checkCollisions();
  ...
  var checkForExtraShip = function () {
  ...
  var checkForEndOfLevel = function () {
    if (rocks.length == 0) {
      switchGameState(GAME_STATE_NEW_LEVEL);
    }
  }

  var gameStatePlayerDie = function () {
  ...
  var updateSaucerMissiles = function () {
  ...
  var renderPlayerShip = function (x, y, rotation, scale) {
    //transformation
  ...
  var renderPlayerMissiles = function () {
    var tempPlayerMissile = {};
    var playerMissileLength = playerMissiles.length - 1;
  ...
  var renderRocks = function () {
    ...
    for (var rockCtr = rocksLength; rockCtr >= 0; rockCtr--) {
      ...
      //Zero Relative 1/2 is if 0.5 * width - 1. Same for height
  ...
  var renderSaucerMissiles = function () {
    var tempSaucerMissile = {};
    var saucerMissileLength = saucerMissiles.length - 1;
...
saucerMissiles:
  ...
  var firePlayerMissile = function () {
  ...
  var createExplode = function (x, y, num) {
    //create 10 particles
  ...
  var splitRock = function (scale, x, y) {
